# Tarea — Agente ReAct Personal con LangChain

## 1. Necesidad personal

Mi necesidad personal es registrar progresivamente el conocimiento sobre
iniciativas pendientes, procedimientos bancarios y personas o equipos
relacionados, porque diariamente necesito recuperar esta información para
resolver consultas e inconvenientes de mi trabajo.

La solución utiliza una denominación bancaria genérica y datos ficticios. No
contiene ni debe recibir información confidencial de una entidad financiera.

## 2. Objetivo

Implementar un **Agente Personal de Conocimiento de Iniciativas y Procedimientos
Bancarios** mediante LangChain y el patrón ReAct. El agente interpreta solicitudes
en lenguaje natural, selecciona herramientas para registrar o consultar
información y responde basándose en los resultados obtenidos, sin inventar
procedimientos, estados, responsables, fechas ni fuentes.
...
- Guardar objetivo, alcance, estado, responsable y fecha.
- Relacionar dependencias, decisiones, documentos, procedimientos y participantes.
- Actualizar varios campos de una iniciativa en una sola solicitud.
- Consultar iniciativas por texto, estado o condición de pendiente.
- Registrar y actualizar procedimientos.
- Diferenciar procedimientos `Borrador` y `Documentado`.
- Buscar información en archivos Markdown y TXT.
- Mantener el contexto durante la sesión.
- Persistir iniciativas y procedimientos en archivos JSON.

Una iniciativa está pendiente cuando su estado no es `Producción` ni
`Finalizada`.
...
Observa el resultado real
        ↓
Responde basándose en la observación
```

El agente no expone la cadena de pensamiento privada. ReAct se demuestra mediante
la selección de herramientas, su ejecución, la observación y la respuesta final.

### Herramientas

| Herramienta | Función |
|---|---|
| `registrar_iniciativa` | Registra una iniciativa sin duplicar nombres. |
| `actualizar_iniciativa` | Actualiza uno o varios campos. |
| `consultar_iniciativas` | Consulta por texto, estado o iniciativas pendientes. |
| `registrar_procedimiento` | Registra un procedimiento. |
| `actualizar_procedimiento` | Actualiza un procedimiento existente. |
| `consultar_procedimientos` | Recupera procedimientos y advierte si son borradores. |
| `buscar_conocimiento` | Busca información en documentos Markdown y TXT. |
...
![El agente solicita los datos faltantes](<Caso de Uso/Captura de pantalla 2026-08-02 a la(s) 8.06.37 p. m..png>)
...
![El agente registra la iniciativa](<Caso de Uso/Captura de pantalla 2026-08-02 a la(s) 8.06.57 p. m..png>)
...
![El agente consulta la iniciativa](<Caso de Uso/Captura de pantalla 2026-08-02 a la(s) 8.07.21 p. m..png>)
...
## 6. Política de transparencia sobre el uso de IA

Se utilizó **Codex**, una herramienta de inteligencia artificial generativa, como
apoyo para revisar y adaptar el código, delimitar el alcance, organizar el caso de
uso y estructurar la documentación.

Se revisó el contenido, se ejecutó y validó el funcionamiento del agente y
asume la responsabilidad sobre la entrega final.
