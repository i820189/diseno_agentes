...
## v4.0 FINAL · Grupo 5 · Proyecto "Tus Eventos" — Asistente de Ventas y Cotización

**Versión:** 4.0 FINAL · **Grupo 5** (Javier, Fernando, John, Jonathan)
...
**Runtime recomendado:** Python + LangGraph/LangChain (`create_agent`) · LLM `gpt-4o-mini` intercambiable vía `.env`
**Canal inicial:** WhatsApp Business Cloud API (público abierto, sin autenticación; ~20 conversaciones/semana) · Streamlit solo demo interna
**Nivel de autonomía:** Semiautónomo y constreñido
**Criticidad:** Media – controlada (12 escenarios con guardrail asignado, §17.3)
...
> **Criterio de diseño:** este card se superpone 1:1 sobre la arquitectura de la
> Tarea S14: agente único, tools deterministas, PostgreSQL como fuente de verdad,
> HITL en el cierre.
...
Construir un agente conversacional de ventas para "Tus Eventos" (alquiler de
dispensadores de bebidas —chopp— y paquetes para eventos en Lima) cuyo **objetivo
primario, ÚNICO y medible, es COTIZAR**:

> **Entregar al cliente una cotización referencial válida**
...
1. Comprender la intención del usuario (taxonomía cerrada: `informarse |
   cotizar_dispensador | cotizar_paquete | reclamar | hablar_con_humano |
   fuera_de_alcance`).
2. Extraer y mantener contexto durante la conversación.
3. Detectar datos faltantes (slots mínimos por línea de producto — §10 —
...
9. Preparar una cotización referencial cuando corresponda (precio SOLO del
   motor de reglas; frase "referencial, sujeto a confirmación" obligatoria).
10. Recopilar los slots de CIERRE **solo tras la aceptación** y registrar una
...
Ejemplo (slots reales del negocio):
...
PREPARE_QUOTE
COLLECT_CLOSING_DATA      (solo tras aceptar la cotización)
CREATE_PRERESERVA         (hold con TTL — única escritura)
ESCALATE
FINISH
```
...
Reglas adicionales del negocio real:
...
Criterios alineados al catálogo real (2 líneas de producto). Pesos iniciales
(ajustables por el negocio, nunca por el LLM):
...
Canal inicial (dato real del negocio):
...
- recibir mensaje;
- asociar `session_id` / `thread_id` (número de WhatsApp);
- identificar `user_id` (mismo número; sin autenticación adicional);
...
- liberación automática de pre-reservas vencidas (job de TTL, no requiere LLM);
...
El System Prompt final del equipo (materialización 1:1 de este card) está en §31.
...
- actualizar estado;
- validar transición;
- persistir short-term memory vía **checkpointer PostgresSaver** (mismo
...
Incluye el **gate de salida** de la cotización: el precio citado debe existir
...
Implementación: **LangSmith**, trazas por `thread_id`/turn (§23).
...
Ejemplo conceptual (dominio real):
...
El estado es **tipado y vive en código**, no en el prompt: aunque el historial
...
class AgentState(TypedDict, total=False):
    ...
    prereserva: dict | None         # hold con TTL (output de crear_prereserva)
    ...
    clarification_retries: int      # contador para max_retries=2 → escalate
...
COLLECTING_CLOSING_DATA     (slots de cierre, solo tras aceptar)
CREATING_PRERESERVA         (hold con TTL — única escritura)
ESCALATING
COMPLETED
ERROR_SAFE_STOP
```
...
Slots mínimos para cotizar, por línea de producto:
...
Campos mínimos para emitir la cotización (además de los slots de la línea):
...
Slots de CIERRE — se piden **ÚNICAMENTE tras la aceptación** de la cotización
...
Prohibido pedir en cualquier etapa: datos de tarjeta o de pago (el pago es
...
El LLM debe devolver únicamente (ejemplo con slots reales):
...
Valores permitidos de `intent` (taxonomía CERRADA):
...
Notas:
- Las modificaciones del pedido y las solicitudes de memoria NO son
  intenciones: son señales que actualizan el estado dentro de la conversación.
- Lo ambiguo o no entendido NO es una intención: se maneja con el umbral
  de confianza siguiente.
...
Si tras `max_retries = 2` aclaraciones sigue sin entenderse → `ESCALATE`
...
Utilizar RAG (**Chroma**; retrieval expuesto como tool) exclusivamente para
...
Fuente: docs en `conocimiento/`, validados por Fernando (negocio real).
...
Información dinámica debe consultarse siempre en la fuente estructurada:
**tablas PostgreSQL**, con **carga inicial migrada desde los Excel del negocio**
(no hay sincronización periódica con Excel; PostgreSQL es la fuente de verdad
desde el día uno).
...
Esquema de cada ítem del catálogo:
...
Reglas de factibilidad (negocio real):
...
Todas las tools de lectura consultan PostgreSQL (§12.2). La ÚNICA tool de
...
Si `covered = false`: rechazo honesto + alternativa (p. ej. recojo en tienda,
...
(Codifica las reglas reales del negocio: 72 h mínimo; feriados con plan de
...
Reglas internas (deterministas, no LLM):
...
(Lee la tabla `calendario` descontando `pre_reservas` activas — control de
doble-booking.)
...
Output (criterios de §3.4, alineados al catálogo real):
...
(Motor de reglas determinista: precio base por ZONA × TEMPORADA + adicionales.
...
Output (con desglose auditable — el gate de salida verifica que el precio
...
(El expediente resumido llega al asesor **por WhatsApp**. Incluye slots,
...
Persistencia por sesión con **checkpointer PostgresSaver** (el mismo
...
Gestión del historial en dos niveles (S9):
...
Invariante clave: el estado es **tipado y vive en código** — aunque el
...
El traspaso corto→largo lo decide un **filtro de negocio** (qué le sirve al
...
- disponibilidad;
- reglas dinámicas;
- datos de pago (JAMÁS se persisten).
...
## 17.0 Reglas duras transversales (no negociables)
...
- confirmar pagos;
- generar o enviar links de pago (exclusivo del asesor);
- confirmar o cancelar reservas definitivas (la única escritura permitida
...
## 17.3 Criticidad — 12 escenarios → guardrail → capa

**Qué podría salir mal → cómo lo controlamos → dónde vive el control.**
...
Escenario complementario: **fuera de alcance** ("dame la receta
...
```text
Channel authentication      (verificación de firma del webhook de Meta;
...
Nota sobre identidad (dato real): el canal es **público abierto, sin
...
PII (alineado a §17.3 escenario 6):
...
Refuerzos: el rol vive en `system`, nunca en `user`; sanitización
...
```text
user_requests_human            (además de buena práctica, exigencia legal
                                   en Perú: Ley 31601 — derecho a humano;
                                   incluye insistencia ≥ 2 veces)
discount_request
commercial_exception
payment_required               (el link de pago SOLO lo genera el asesor)
complaint_complex              (reclamos: empatía, pedir nombre, número de
                                   pedido y descripción; NUNCA prometer
                                   compensaciones)
source_conflict
insufficient_information_after_clarification   (max_retries = 2)
tool_failure_without_fallback
security_risk
agent_confidence_too_low
max_agent_steps_exceeded
quote_accepted (CIERRE FELIZ: expediente completo + pre-reserva creada →
   el asesor genera el link de pago y confirma la reserva definitiva)
```

La derivación viaja **por WhatsApp** al asesor e incluye un resumen
estructurado (expediente) — el cliente no repite su información.

Ejemplo (slots reales):
...
Implementación: **LangSmith** — trazas por `thread_id` / turn (decisión del
...
- user_id anonimizado cuando aplique (número de WhatsApp con etiqueta reversible);
- estado;
- objetivo;
- tool;
- input sanitizado;
...
PII en logs: etiquetas reversibles (no X), retención mínima (§18).
...
memory consent
escalation
prereserva preconditions & TTL
output gate (precio = output de tool + frase "referencial")
```
...
KPIs de control del equipo (observabilidad por paso):
...
unvalidated_price_rate: "0%"
quote_format_pass_rate: "100%"
```
...
El profesor debe poder superponer este diagrama sobre la arquitectura de la
Tarea S14 y ver LO MISMO:
...
Por qué así (sustento): un solo PostgreSQL concentra ground truth, checkpoints
...
│   ├── availability.py
│   ├── pricing.py
│   ├── prereserva.py        (hold TTL — única escritura, §13.11)
│   ├── memory.py
│   └── escalation.py
│
├── rag/
...
PREPARE_QUOTE
COLLECT_CLOSING_DATA     (solo tras aceptar la cotización)
CREATE_PRERESERVA        (hold TTL, precondiciones §13.11)
ASK_MEMORY_CONSENT
SAVE_MEMORY
ESCALATE
FINISH
...
def handle_message(message, state):
    ...
    # Cotización: precio SOLO del motor de reglas + gate de salida
    ...
    # Cierre feliz: aceptó → slots de cierre → pre-reserva → asesor
...
## 31.1 System Prompt — VERSIÓN FINAL del equipo (materialización 1:1 del card)

Este es el prompt canónico del Grupo 5 (se adjunta también en la Tarea S14);
...
- comprende al menos las intenciones definidas (taxonomía cerrada de 6);
...
- usa `AgentState`;
- valida transiciones;
- persiste short-term state (PostgresSaver; slots sobreviven a trim/summary).
...
- `crear_prereserva` respeta precondiciones, transacción e idempotencia por `quote_id`.
...
- se usa solo para conocimiento estable (FAQ/políticas en Chroma);
- no reemplaza Ground Truth (PostgreSQL).
...
- tool calls quedan registradas (LangSmith por thread/turn).
...
discount request
source conflict
closing-data before acceptance (debe bloquearse)
prereserva TTL expiry & idempotency
payment requested in chat (debe escalar)
```
...
## Caso 6 — Cierre feliz + pre-reserva
...
[ ] crear_prereserva transaccional con TTL implementada (única escritura)
[ ] Gate de salida de cotización (precio de tool + frase "referencial")
[ ] RAG separado de Ground Truth
[ ] ReAct controlado implementado
[ ] Guardrails implementados (incl. tabla de 12 escenarios §17.3)
[ ] Memory consent implementado
[ ] Checkpointer PostgresSaver + trim_tokens + summary implementados
[ ] Human escalation implementado (incl. cierre feliz por WhatsApp)
[ ] Retry/error handling implementado
[ ] Logging/tracing implementado (LangSmith)
...
7. Utility Ranking
8. Quote + output gate            (precio de tool + frase "referencial")
9. Prereserva (hold TTL)         
...
La carga inicial de datos se hace **migrando los Excel del negocio a las
...
*v4.0 — consolidación del equipo sobre la base técnica de John; trazabilidad de
cambios en `Agentic_Profile_Card_v4.0_Consolidado.md`. Grupo 5 (Javier, Fernando,
John, Jonathan). Este documento es la fuente de verdad del proyecto: la
arquitectura de la Tarea S14 y el System Prompt final (§31.1) son su
materialización 1:1.*
